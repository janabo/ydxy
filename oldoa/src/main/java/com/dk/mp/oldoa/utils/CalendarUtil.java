...
public class CalendarUtil {
	 ...
	 * @return 是或否
	 */
	public static boolean comparedDate(Date d1, Date d2) {
		boolean bool = true;
		try {
			if (d1.before(d2)) {
				bool = true;
	...
	public static String getTimeAfterMins(int mins, String startTime) {
	...
	public static String getDate(String date) {
		...
		Date d;
		try {
			d = formatYMD.parse(date);
			calendar.setTime(d);
			dateS = NumberHelper.LeftPad_Tow_Zero((calendar.get(Calendar.DAY_OF_MONTH))) + "日 "
					+ TimeUtils.getWeekDayInt2Str(TimeUtils.getWeek(date));
		} catch (ParseException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
