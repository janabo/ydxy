...
public class WheelView extends View {
	...
	private static final int VALUE_TEXT_COLOR = 0xFF212121;

	/** Items text color */
	private static final int ITEMS_TEXT_COLOR = 0xFFc9c9c9;
	...
	private int calculateLayoutWidth(int widthSize, int mode) {
		...
		if (maxLength > 0) {
            double textWidth =  Math.floor(Layout.getDesiredWidth("0", itemsPaint));
//			float textWidth = FloatMath.ceil();
		...
		if (label != null && label.length() > 0) {
            labelWidth = (int)  Math.floor(Layout.getDesiredWidth(label, valuePaint));
