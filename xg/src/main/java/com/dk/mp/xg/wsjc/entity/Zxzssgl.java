package com.dk.mp.xg.wsjc.entity;

/**
 * 作者：janabo on 2017/2/24 15:57
 */
public class Zxzssgl {
    private String ssl;//宿舍楼
    private String lc;//楼层
    private String fjh;//楼层
    private String ssq;//楼层
    private String xq;//楼层
    private String xsid;//楼层
    private String xqid;//楼层
    private String ssqid;//楼层
    private String sslid;//楼层
    private String cwh;//楼层
    private String fx;//楼层
    private String zsf;//楼层
    private String cws;//楼层
    private String tzyy;//楼层
    private String tzyyid;//楼层
    private String bz;//楼层

    public String getSsl() {
        return ssl;
    }

    public void setSsl(String ssl) {
        this.ssl = ssl;
    }

    public String getLc() {
        return lc;
    }

    public void setLc(String lc) {
        this.lc = lc;
    }

    public String getFjh() {
        return fjh;
    }

    public void setFjh(String fjh) {
        this.fjh = fjh;
    }

    public String getSsq() {
        return ssq;
    }

    public void setSsq(String ssq) {
        this.ssq = ssq;
    }

    public String getXq() {
        return xq;
    }

    public void setXq(String xq) {
        this.xq = xq;
    }

    public String getXsid() {
        return xsid;
    }

    public void setXsid(String xsid) {
        this.xsid = xsid;
    }

    public String getXqid() {
        return xqid;
    }

    public void setXqid(String xqid) {
        this.xqid = xqid;
    }

    public String getSsqid() {
        return ssqid;
    }

    public void setSsqid(String ssqid) {
        this.ssqid = ssqid;
    }

    public String getSslid() {
        return sslid;
    }

    public void setSslid(String sslid) {
        this.sslid = sslid;
    }

    public String getCwh() {
        return cwh;
    }

    public void setCwh(String cwh) {
        this.cwh = cwh;
    }

    public String getFx() {
        return fx;
    }

    public void setFx(String fx) {
        this.fx = fx;
    }

    public String getZsf() {
        return zsf;
    }

    public void setZsf(String zsf) {
        this.zsf = zsf;
    }

    public String getCws() {
        return cws;
    }

    public void setCws(String cws) {
        this.cws = cws;
    }

    public String getTzyy() {
        return tzyy;
    }

    public void setTzyy(String tzyy) {
        this.tzyy = tzyy;
    }

    public String getTzyyid() {
        return tzyyid;
    }

    public void setTzyyid(String tzyyid) {
        this.tzyyid = tzyyid;
    }

    public String getBz() {
        return bz;
    }

    public void setBz(String bz) {
        this.bz = bz;
    }
}
